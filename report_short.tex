...
\title{\textbf{Delta Hedging under Stochastic Volatility: When Structure Beats Learning}}
...
persistence baseline (rv21) remains strongest at the 21-day horizon because
of volatility clustering, highlighting that short-horizon prediction is largely
dominated by persistence, leaving limited room for further model improvement.
...
biases (anchor targets) or reduced action spaces, it fails to outperform simple
deterministic control around the analytic solution.

\textbf{Takeaway.} RL methods fail to generalize to real market data in this
setup. Structure plus good inputs beats learning from scratch.
