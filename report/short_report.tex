...
trailing 21-day window $[t\!-\!20,\,t]$ is adjacent to the forward target
$[t\!+\!1,\,t\!+\!21]$ and highly informative under volatility clustering
(Nifty Hurst exponent $H\approx0.65$), making persistence a near-optimal
predictor at this horizon.  This strong short-horizon persistence limits
the room available to any model, including LSTM, to outperform simple
...
computed model prices for 51 held-out Nifty options and measured MAE
against market prices (Table~\ref{tab:pricing}).

\begin{table}[H]
\centering
\caption{Option pricing MAE on Nifty~50 (lower is better).}
\label{tab:pricing}
\begin{tabular}{lc}
\toprule
...
($-31\%$ MAE: $30.3$ vs $43.6$), a \textbf{positive} result for the
learned model.  rv21 and implied volatility are nearly tied at the top;
the gap between rv21 and LSTM at the 21-day horizon again reflects strong
short-horizon volatility persistence, not a limitation of the neural
architecture.  India VIX's near-parity with rv21 confirms that the
variance risk premium (the gap between $\mathbb{Q}$ and $\mathbb{P}$
measures) is small relative to the persistence effect at this horizon.
...
BSM-consistent policy in simulation: MAE~0.723, BSM-correlation~\textbf{0.963}.
...
approaches competitive real-path performance, although it still does not
beat BSM-LSTM (96.1) or Partial-BSM-LSTM (88.3).
Closing this remaining gap is left for future work.
...
for training.  The constant-$\sigma$ RL baseline in this environment
achieves MAE~$1.435$; the LSTM-RL agent achieves MAE~$\mathbf{0.407}$.

\begin{table}[H]
\centering
\caption{LSTM-RL in its training environment (natural habitat), $n=1000$ paths.}
\label{tab:habitat}
\begin{tabular}{lcc}
\toprule
Strategy & MAE & CVaR$_{95}$ \\
\midrule
Constant-$\sigma$ RL & 1.435 & $-5.08$ \\
\textbf{LSTM-RL (PPO)} & \textbf{0.407} & $\mathbf{-1.85}$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Takeaway.}  LSTM-RL achieves a \textbf{72\% reduction in MAE} and a
\textbf{64\% reduction in CVaR$_{95}$} versus the constant-$\sigma$ RL
baseline in its training environment ($p<0.001$, paired bootstrap).  This is the
