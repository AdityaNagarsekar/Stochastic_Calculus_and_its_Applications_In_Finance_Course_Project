...
\]
For ATM Nifty options ($S\!\approx\!22{,}000$, $\tau=21/252$): $\approx$ \textbf{25 points per 1 pp vol error}.\\[0.3em]
{\small GARCH error $\approx1.9$ pp $\Rightarrow$ \textbf{49 pts};\quad LSTM $\approx1.1$ pp $\Rightarrow$ \textbf{29 pts};\quad rv21 $\approx0.4$ pp $\Rightarrow$ \textbf{11 pts}. GARCH$\to$LSTM saves $\approx$\textbf{20 pts/option}.}
...
\begin{itemize}\itemsep2pt
  ...
  \item 6 features: rv5, rv21, rv\_ratio, log-return, |return|, mom5
...
\]
Intended to isolate the controllable error $\varepsilon_t+G_t = (\delta-\Delta^\star)\Delta S$; subtraction doubles the noise instead.\\[0.3em]
\textbf{In practice, B is \emph{worse} than A} (MAE $0.926$ vs $0.830$).

\medskip
\textbf{Problem:} Near convergence, gradient $\propto (\varepsilon_t - G_t)\!\approx\!-2G_t$ — \emph{pure noise}, destabilising training rather than converging.
